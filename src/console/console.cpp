...
MenuNode* g_autoToolNode = nullptr;         // AutoToolのメニューノードへの参照
...
void UpdateMenuDisplay() {
    ...
    for (int i = startIdx; i <= endIdx; ++i) {
        ...
        if (current == &g_menuTree) {
            ...
            } else if (name == L"AutoTool") {
                itemColor = GetAutoToolEnabled() ? RGB(0, 255, 0) : RGB(255, 0, 0);
...
void InitMenu() {
    ...
    LoadAutoToolConfig();       // AutoTool設定のロード。実装理由：起動時に前回のON/OFF設定を引き継ぐため。
    ...
    g_menuTree = { L"ROOT", {
        ...
        { L"AutoTool", { { L"..", {} }, { GetAutoToolMenuName(), {} } } },
    ...
    for (auto& child : g_menuTree.children) {
        ...
        } else if (child.name == L"AutoTool") {
            g_autoToolNode = &child.children[1];
...
void ToggleMenuNode(const std::wstring& parentName, MenuNode* toggleNode) {
    ...
    } else if (parentName == L"AutoTool") {
        bool nextState = !GetAutoToolEnabled();
        SetAutoToolEnabled(nextState);
        AddLog(nextState ? L"[AutoTool] Enabled" : L"[AutoTool] Disabled");
        if (toggleNode) {
            toggleNode->name = GetAutoToolMenuName();
        }
        SaveAutoToolConfig();
